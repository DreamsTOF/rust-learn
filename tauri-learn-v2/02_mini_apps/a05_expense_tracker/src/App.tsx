...
export default function App() {
  ...
  async function refresh(db: Database) {
    // === 步骤 1 ————————————————————————————————————————————
    // TODO: 查询全部支出，按 id 倒序（最新在前）
    // 提示: db.select<Expense[]>("SELECT id, title, amount, category, created_at AS createdAt FROM expenses ORDER BY id DESC")
    const rows = await db.select<Expense[]>("SELECT 1"); // ← 替换成你的代码
    setExpenses(rows);

    // === 步骤 2 ————————————————————————————————————————————
    // TODO: 月度合计——按月分组求和（strftime 取 YYYY-MM，SUM 求和）
    // 提示: db.select<MonthTotal[]>("SELECT strftime('%Y-%m', created_at) AS month, SUM(amount) AS total FROM expenses GROUP BY month ORDER BY month DESC")
    const months = await db.select<MonthTotal[]>("SELECT 1"); // ← 替换成你的代码
    setTotals(months);
  }
  ...
  async function remove(id: number) {
    ...
    try {
      // === 步骤 4 ————————————————————————————————————————————
      // TODO: 按 id 删除
      // 提示: db.execute("DELETE FROM expenses WHERE id = $1", [id])
      await db.execute("SELECT 1", [id]); // ← 替换成你的代码
      await refresh(db);
      setStatus("已删除");
    } catch (e) {
      setStatus(`删除失败: ${e}`);
