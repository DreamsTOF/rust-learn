...
export default function App() {
  ...
  useEffect(() => {
    const un = getCurrentWebview().onDragDropEvent(async (event) => {
      ...
      // === 步骤 2 ————————————————————————————————————————————
      // TODO: 支持"拖文件夹"——遇到非图片路径时调 list_images 列出其中的图片
      // 提示: invoke<string[]>("list_images", { dir: p })
      //       （示例先实现"拖图片"，文件夹分支留给练习）
      setImages(paths.filter(isImagePath));
      setIndex(0);
      setStatus(`${paths.length} 个文件`);
    });
    return () => {
      un.then((f) => f());
    };
  ...
  async function toggleAlwaysOnTop() {
    ...
    setAlwaysOnTop(next);
    // === 步骤 3 ————————————————————————————————————————————
    // TODO: 调用窗口的 setAlwaysOnTop(next)
    // 提示: await getCurrentWindow().setAlwaysOnTop(next);
  }
  ...
  async function toggleFullscreen() {
    ...
    setFullscreen(next);
    // === 步骤 4 ————————————————————————————————————————————
    // TODO: 调用窗口的 setFullscreen(next)
    // 提示: await getCurrentWindow().setFullscreen(next);
  }
