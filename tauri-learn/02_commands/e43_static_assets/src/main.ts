...
// TODO: 完成填空后取消注释（asset 协议：把磁盘路径转成可加载 URL）
...
infoBtn!.addEventListener("click", async () => {
  try {
    // === 步骤 2: 调用 resource_info ————————————————————————————
    // TODO: const dir = await invoke<string>("resource_info");
    //       resultEl!.textContent = `resource 目录: ${dir}`;
    //       resultEl!.className = "status ok";
    // 提示: invoke 来自 "@tauri-apps/api/core"；命令返回路径字符串
    // 当前为占位（完成填空后替换）
    resultEl!.textContent = "（resource_info 待补全）";
