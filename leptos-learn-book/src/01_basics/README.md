...
本模块提供两个服务。**只需打开 mdbook，页内 iframe 就是完整的分屏编辑器。**
...
启动两个终端后，打开 mdbook，每个练习页面底部的沙箱区块就是一个**完整的分屏编辑器**：

- **左半边** — 代码编辑器（直接修改）
- **右半边** — 实时预览（点击 Save & Run 后更新）
- **快捷键** — `Ctrl+S` 保存

也可打开 [http://localhost:3002/?e=01](http://localhost:3002/?e=01) 全屏使用。
