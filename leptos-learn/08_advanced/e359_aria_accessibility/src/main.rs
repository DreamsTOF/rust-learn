...
fn BadButton() -> impl IntoView {
    // TODO: 这是一个没有 ARIA 属性的按钮，div 模拟的按钮无任何无障碍信息
    view! {
        <div
...
fn GoodButton() -> impl IntoView {
    ...
    // - tabindex="0" 使其可获得焦点
    // - on:keydown 处理 Enter/Space 键
    view! {
        <div
            style="display: inline-block; padding: 10px 20px; background: #1a73e8; color: white; border-radius: 4px; cursor: pointer;"
...
fn AlertMessage(message: String, is_visible: bool) -> impl IntoView {
    // TODO: 使用 aria-live="polite" 让屏幕阅读器播报动态消息
    // 使用 role="alert" 定义 alert 角色
    if is_visible {
        view! {
...
fn AccessibleInput(
    ...
    // TODO: 使用 attr:aria-label、attr:aria-describedby 等
    view! {
        <div style="margin: 8px 0;">
            ...
            />
            // TODO: 添加 aria-describedby 指向帮助文本
    ...
    view! {
        <div style="max-width: 700px; margin: 2rem auto; font-family: system-ui, sans-serif; padding: 0 1rem;">
            ...
            <section style="margin: 2rem 0; padding: 1.5rem; border: 1px solid #e0e0e0; border-radius: 8px;">
                ...
                // TODO: AlertMessage 组件需要 aria-live="polite" 和 role="alert"
                // 注意: 传递 signal 值需要使用 .get() 调用获取当前值
                <AlertMessage message={alert_msg.get()} is_visible={show_alert.get()} />
            </section>

            <section style="margin: 2rem 0; padding: 1.5rem; border: 1px solid #e0e0e0; border-radius: 8px;">
                <h2>"⌨️ 可访问表单"</h2>
                // TODO: 传入 described_by 参数绑定帮助文本 ID
                <AccessibleInput label="用户名" />
            </section>

            // TODO: 添加全局样式，为 :focus-visible 元素添加蓝色外环
            // 提示: 在 view! 中使用 <style>{":focus-visible { outline: 3px solid blue; }"}</style>
        </div>
    }
}
