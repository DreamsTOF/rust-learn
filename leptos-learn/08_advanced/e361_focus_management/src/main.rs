...
fn Exercise() -> impl IntoView {
    ...
    // === 步骤 1: 自动对焦 ————————————————
    // 使用 Effect::new 在组件挂载后自动聚焦到 input_ref
    Effect::new(move || {
    ...
    // === 步骤 2: 焦点陷阱键盘处理 ————————————
    // TODO: 当 Tab 键按下时，调用 trap_focus(container, shiftKey) 实现循环
    // 1. 检查 ev.key() == "Tab"，否则返回
    // 2. ev.prevent_default() 阻止默认行为
    // 3. 获取 trap_ref.get()，将其传入 trap_focus(trap.as_ref(), ev.shift_key())
    let on_trap_keydown = move |_ev: KeyboardEvent| {
        // === 你的代码开始 ===
        todo!("实现焦点陷阱逻辑");
        // === 你的代码结束 ===
    };
