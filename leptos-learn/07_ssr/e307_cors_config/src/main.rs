// ============================================================
// 练习 e307: CORS 配置 (Cross-Origin Resource Sharing)
//
// 核心知识点:
//   - CORS 概念与跨域请求流程（简单请求 vs 预检请求）
...
// 难度: ⭐⭐ (关键 TODO 已标记)
//
// 说明: 本练习展示如何在 Axum 中配置 CORS 策略。
// 你将看到 CorsLayer 的常见配置方式，并通过代码示例
// 理解预检请求的工作流程。
// ============================================================
        ...
        <div style="padding: 1.5rem; font-family: system-ui, sans-serif; max-width: 48rem; margin: 0 auto;">
            ...
            <section style="margin: 1.5rem 0;">
                ...
                <ul style="line-height: 1.8; color: #334155;">
                    <li>"🌐 <strong>简单请求</strong>：GET/HEAD/POST，Content-Type 为 text/plain、multipart/form-data 或 application/x-www-form-urlencoded"</li>
                    <li>"⚠️ <strong>预检请求</strong>：使用 OPTIONS 方法先询问服务器是否允许实际请求"</li>
...
let app = Router::new()
    ...
    // ⭐⭐ TODO 1: 添加 CorsLayer 中间件，允许所有来源
    // 提示: 使用 CorsLayer::permissive()
    .layer(_________________________________);"
                </pre>
                <p style="color: #fbbf24; margin-top: 1rem;">
                    "⭐ TODO 2: 完成受限 CORS 配置（仅允许特定源和方法）："
                </p>
                <pre style="font-size: 0.875rem; overflow-x: auto;">
"let cors = CorsLayer::new()
    ...
    // ⭐⭐ 允许的 HTTP 方法:
    // .allow_methods(___________________________)
    // ⭐⭐ 允许的自定义头部:
    // .allow_headers(___________________________)
    // ⭐⭐ 允许发送凭据 (Cookie/Authorization):
    // .allow_credentials(_______________________);"
                </pre>
                <p style="color: #94a3b8; font-size: 0.85rem; margin-top: 0.5rem;">
                    "💡 提示: use tower_http::cors::{CorsLayer, Any, AllowOrigin};"
                    "allow_methods(Any) 允许所有方法；allow_methods([Method::GET, Method::POST]) 限制特定方法。"
                    "allow_credentials(true) 允许携带 Cookie 和 Authorization 头部。"
                </p>
            </section>
...
// ⭐⭐ TODO 3: 用 Axum 处理 OPTIONS 请求的预检路由
// 提示: Router::new().route(\"/api/data\", options(|| async { StatusCode::OK }))"
                </pre>
            </section>
