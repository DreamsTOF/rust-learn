        ...
        <div style="padding: 1.5rem; font-family: system-ui, sans-serif; max-width: 48rem; margin: 0 auto;">
            ...
            <section style="margin: 1.5rem 0;">
                ...
                <ul style="line-height: 1.8; color: #334155;">
                    <li>"🎯 State 提取器：类型安全、编译期检查的状态访问方式"</li>
                    <li>"🔌 Extension：基于请求扩展的轻量级状态传递"</li>
                    <li>"📋 AppState：将多个资源合并为一个结构体"</li>
                </ul>
            </section>
...
let config = AppConfig { max_users: 100, app_name: \"MyApp\".into() };
...
#[derive(Clone)]
struct AppConfig { max_users: usize, app_name: String, }

#[derive(Clone)]
struct AppState { db_pool: PgPool, config: Arc<AppConfig>, }
