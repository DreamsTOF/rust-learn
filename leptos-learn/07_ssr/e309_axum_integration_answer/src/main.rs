...
"use axum::{Router, Json, http::StatusCode, routing::{get, post}};
use serde_json::json;
...
// 主路由：嵌套 API + Leptos SSR fallback
let app = Router::new()
    .nest(\"/api\", api_router)
                            ...
                            view! {
                                <tr>
                                    ...
                                    <td style="padding: 0.5rem; border-bottom: 1px solid #e2e8f0;">
                                        <span style="background: {(if method == "GET" { "#dbeafe" } else { "#fef3c7" })}; padding: 0.15rem 0.5rem; border-radius: 4px; font-size: 0.8rem;">
                                            {method}
                                        </span>
                                    </td>
