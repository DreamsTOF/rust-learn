// ============================================================
// 练习 200: real_world_dashboard — 综合异步仪表盘
//
// 目标: 综合运用 Resource/Suspense/Transition/错误处理/定时刷新
//
// 难度: ⭐⭐⭐⭐
// 核心知识点: 综合异步仪表盘
//
// TODO:
//   1. 实现 sleep + 伪随机工具函数
//   2. fetch_users / fetch_stats: 模拟 API 调用(偶尔失败)
//   3. 使用 Resource + Suspense 加载数据
//   4. 错误处理: 显示错误状态
//   5. 定时刷新: 每 5 秒自动重新加载
// ============================================================
...
async fn fetch_users() -> Result<Vec<User>, String> {
    sleep(700).await;
    if pseudo_random() < 0.15 {
        return Err("用户服务暂时不可用".into());
    }
    Ok(vec![
...
fn Exercise() -> impl IntoView {
    // 刷新触发器: 改变此信号值即可触发 Resource 重新加载
    ...
    let stats = Resource::new(
        move || refresh_trigger.get(),
        |_| async { fetch_stats().await },
    );

    // 定时刷新: 每 5 秒递增 refresh_trigger
    let trigger_clone = refresh_trigger;
    set_interval(
        move || {
            trigger_clone.update(|v| *v += 1);
        },
    ...
    let user_list = move || {
        match users.get() {
            ...
            Some(Err(e)) => {
                vec![view! { <tr><td style="color:red">"⚠ 错误: " {e}</td></tr> }.into_any()]
            }
            None => vec![],
        }
    };
    ...
    view! {
        <div>
            ...
            <button on:click=manual_refresh>
                "🔄 手动刷新"
            </button>
...
//
// ### 核心思路
// - `refresh_trigger: RwSignal<u32>` 作为刷新信号
// - Resource 的 deps 依赖 `refresh_trigger`, 值变化时重新 fetch
// - `set_interval` 每 5 秒递增触发, 按钮手动递增触发
// - Resource 返回 `Result<T, String>`, 视图区分 Ok/Err
//
// ### 关键代码
// ```rust
// let refresh_trigger = RwSignal::new(0u32);
...
//     |_| async { fetch_users().await },
// );
// set_interval(move || refresh_trigger.update(|v| *v += 1), 5_secs);
// ```
//
// ### 知识点
// - Resource deps 驱动自动重载
// - `set_interval` 定时刷新
// - `Result<T, E>` 在 Resource 中的错误处理
// - Suspense + 手动/自动刷新组合
//
// </details>
